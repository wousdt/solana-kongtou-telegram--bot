...
```bash
curl -i https://bot.apibot.xin/health
```
...
私有镜像可以避免直接公开源码，但拥有服务器 root 与 Docker 权限的人仍可能导出并分析镜像。授权管理端私钥必须只保存在授权管理员控制的服务器上，不能放进本仓库、机器人镜像或用户 `.env`。
